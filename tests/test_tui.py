...
def test_large_terminal_paste_stores_marker_without_modal(tmp_path: Path) -> None:
    """Terminal paste behaves like clipboard paste, including on Windows Terminal."""
    app, agent, _store = _app(tmp_path)
...
def test_windows_f4_bypasses_terminal_for_large_clipboard_paste(
    monkeypatch, tmp_path: Path,
) -> None:
    app, agent, _store = _app(tmp_path)
    pasted = "native clipboard ✓\n" * 10_000
...
def test_tui_composer_buttons_do_not_overlap_footer(tmp_path: Path) -> None:
    ...
    async def scenario() -> None:
        async with app.run_test(size=(100, 32)):
            ...
            buttons = [
                app.query_one(f"#{name}", Button)
                for name in ("paste", "newline", "cancel", "send")
            ]

            hint = str(app.query_one("#composer-hint").render())
            assert hint == "Enter/F2 send · Ctrl+J new line · F4 paste"
