...
| Target | Separate organization project plus optional `neo-py` adapter |
...
eventually own its detailed core roadmap; this file should then track only the Neo
...
- [ ] Move the detailed standalone-system requirements into the new project's design
  documents and retain only Neo integration requirements here.
...
- [ ] Add a small model protocol and a `neo-py` provider adapter without importing Neo
...
## Milestone 3 - Read-only Neo integration

- [ ] Add an optional adapter/registration seam to Neo's hardcoded tool registry.
...
- [ ] Verify Neo works unchanged when the package/database is missing or disabled.

## Milestone 4 - Controlled write integration

- [ ] Implement separate tools for ingestion and explicit task state changes.
- [ ] Integrate write tools with Neo's approval/event model after cancellation and
