...
known local credential files (`.env` variants, `*.local.yaml`, `.hal/auth.json`,
and legacy `.neo/auth.json`), and already-staged paths outside the requested set. Its result
